...
# Design Graph — 실행 단위 연결 구조 설계 (Codex CLI판)

> **Codex CLI 전용 스킬이다** — 절차가 Codex의 구성 요소(AGENTS.md·
> `$HOME/.agents/skills`·`codex exec`·approval/sandbox 정책)를 전제한다.
...
4. **Deterministic 기본** — 룰·코드로 고정할 수 있는 라우팅·검증은 LLM에게
   ...
   단다. 골격은 deterministic 스크립트, Node 내부만 agentic이 기본형이다.
...
6. **Minimal 시작** — Node 하나가 늘 때마다 State·Routing·Failure 문제가 곱으로
   는다. 단일 세션 + deterministic 스크립트로 충분하면 그래프를 만들지 않는
   것이 정답이며, 그렇게 보고한다.
...
- **기존 자산**: 프로젝트/사용자 스킬(`$HOME/.agents/skills` 등)·스크립트·
  Makefile/CI 잡·AGENTS.md에 정의된 절차. 이미 있는 것을 새 Node로 다시
  만들지 않는다.
...
- **런타임 기본기**: 현재 Codex CLI가 기본 제공하는 오케스트레이션 수단 —
  `codex exec`(비대화 실행, 스크립트에서 체이닝 가능)·approval/sandbox
  정책·AGENTS.md 자동 로딩·skills 시스템 등. 버전에 따라 다르므로 이번
  세션에서 확인 가능한 것 기준, 확인 못 한 것은 "미확인" 표기.

Node는 Agent만이 아니다 — 함수·스크립트·검증기·CI·사람도 Node다. LLM이
필요 없는 자리에 Agent 호출을 두지 않는다.
...
합류(전부 대기)는 정말 전 결과가 함께 필요한 곳에만 둔다. 모든 Node/Edge에
...
- Codex에서 Node 간 전달은 보통 파일이 매체다 — Edge마다 산출 파일의 경로·
  형식(JSON 스키마·마크다운 골격)을 확정하고, 다음 Node가 그것만 읽게 한다.

**(c) State** — Node 사이를 흐르는 작업 상태의 정의와 소유자(어느 파일이
단일 진실원인가). 두 Node가 같은 상태를 각자 들고 있게 하지 않는다.

### 4. Governance 배치

1단계의 Invariant마다 게이트를 배치한다:

- **사전 게이트가 기본**: `Node → 게이트 → 실행`. 2단계 인벤토리에서 확인된
  런타임 강제 수단만 쓴다 — approval 정책·sandbox 설정(실행 전 차단)·
  오케스트레이션 스크립트의 검증 스텝·CI 게이트. `실행 → 사후 검증`만 있는
  Invariant는 설계 결함으로 보고하고 승격안을 제시한다.
- **폭주 방지**: 재시도 상한·루프 탈출 조건·호출 횟수/비용 상한을
  오케스트레이션 스크립트에 명시한다. BLOCKED(외부 조건)에서는 재시도하지
  않는다 — 무한 retry의 전형적 원천. Budget 소진은 기계적으로 FAILED가
  아니라 상황에 따라 FAILED/BLOCKED/ESCALATED로 분류한다. 그래프가 스스로를
  수정·확장할수록 게이트는 더 강해야 한다.
- **사람 게이트**: 1단계에서 나온 사람 개입 지점을 그래프 상 위치로 박는다 —
  "필요하면 물어본다"가 아니라 스크립트가 어느 Edge에서 멈추는지를.
...
- **Kill Criteria**: 무엇이 보이면 그래프를 줄이거나 되돌리는가?
  (증상 예: Contract 위반이 잦아 Edge마다 땜질, 합류 대기가 병렬 이득을
  잠식, 단일 세션으로 했을 때보다 느리거나 비쌈, 디버깅 불가)
- **재검토 시점**: 언제 다시 보는가? (Node 추가·모델 교체 시 Edge Contract
  재검토)

승인 후 파일을 생성한다 — 오케스트레이션 스크립트·Node별 스킬·Edge 산출
파일 스키마·CI 설정 등, 설계에서 확정된 것만. 생성한 파일 목록과 각 파일이
그래프의 어느 Node/Edge/게이트인지 요약을 보고한다.

## 산출물이 아닌 것

- Node **내부**를 설계하지 않는다 — 개별 Node의 지침·도구·검증은
  design-harness의 몫이다. 이 스킬은 연결·계약·배치만 다룬다.
- Multi-Agent를 전제하지 않는다 — 설계 결과가 "세션 하나 + 스크립트"라면
